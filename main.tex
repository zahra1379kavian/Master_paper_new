...
\begin{figure}[!p]
    ...
    \end{minipage}
    \caption{Validation displays for the optimisation framework. \textbf{A}, The paired plot compares consecutive-trial changes in the optimised neural projection with consecutive-trial changes in reaction time. Positive behaviour-minus-projection values mark cases where the behaviour change is larger than the projection change. \textbf{B}, The voxel-stability panels compare selected voxels with size-matched non-selected motor-area control samples and show the distribution of voxel beta-variability rankings.}
...
\begin{figure}[!p]
    ...
    \end{minipage}\hfill
    \caption{Exploratory GVS-related edge changes in the optimised vigour and task-activation networks. \textbf{A}, FDR-thresholded active--sham mutual-information edge changes are shown for the optimised vigour network. \textbf{B}, The same mutual-information analysis is shown for the task-activation network. \textbf{C}, FDR-thresholded active--sham Pearson edge changes are shown for the task-activation network. Changes were pooled across subjects, medication states, runs, and active GVS waveforms. Orange edges mark increased statistical dependence or linear correlation, and blue edges mark decreased dependence or linear correlation. Colour intensity scales with the mean edge-change magnitude. Nodes are labelled by ROI and grouped by anatomical sector.}
