...
Voxelwise overlap between the vigour network and the standard task-activation map was quantified using the number of shared voxels, the Dice coefficient, and the Jaccard index. ROI-level anatomical coverage was summarised as the percentage of each atlas-defined ROI occupied by the vigour network, the task-activation map, and their same-voxel overlap.
...
\subsection{Spatial distribution of the vigour network}
...
As a control analysis, we repeated the GVS edge-change analysis using the task-activation map instead of the vigour network (Figure~\ref{fig:gvs_edge_changes}B,C). GVS also produced significant edge changes in this task-activation network. However, the detailed pattern was different: the task-activation map showed \(134\) significant mutual-information edges and \(148\) significant Pearson edges, with only \(14\) shared mutual-information edges and \(16\) shared Pearson edges overlapping the significant vigour network edges. This supports a cautious interpretation: GVS affects task-active circuitry in general, but the vigour network shows a partly distinct pattern of GVS-sensitive connectivity.
...
\begin{figure}[!p]
    ...
    \end{minipage}\hfill
    \caption{Exploratory GVS-related edge changes in the vigour network and task-activation network. \textbf{A}, FDR-thresholded active--sham mutual-information edge changes are shown for the vigour network. \textbf{B}, The same mutual-information analysis is shown for the task-activation network. \textbf{C}, FDR-thresholded active--sham Pearson edge changes are shown for the task-activation network. Changes were pooled across subjects, medication states, runs, and active GVS waveforms. Orange edges mark increased statistical dependence or linear correlation, and blue edges mark decreased dependence or linear correlation. Colour intensity scales with the mean edge-change magnitude. Nodes are labelled by ROI and grouped by anatomical sector.}
