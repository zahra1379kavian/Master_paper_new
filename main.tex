...
As a control analysis, we repeated the GVS edge-change analysis using the task-activation map instead of the optimised vigour network (Figure~\ref{fig:gvs_edge_changes}B,C). GVS also produced significant edge changes in this task-activation network. However, the detailed pattern was different: the task-activation map showed \(134\) significant mutual-information edges and \(164\) significant Pearson edges, with only \(14\) shared mutual-information edges and \(16\) shared Pearson edges overlapping the significant vigour network edges. This supports a cautious interpretation: GVS affects task-active circuitry in general, but the optimised vigour network shows a partly distinct pattern of GVS-sensitive connectivity.
