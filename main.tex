...
After trial- and run-level quality control, \(18\) participants, \(35\) sessions, \(70\) runs, and \(5{,}463\) Go trials were retained for behavioural and neural analyses. Mean RT was \(387.2 \pm 65.8\)~ms in the OFF-medication session and \(375.5 \pm 64.7\)~ms in the ON-medication session, and the subject-paired medication effect on mean RT was small (Supplementary Figure~S2). Notably, despite lower mean RT ON-medication, consecutive-trial RT variability was higher ON than OFF (\(4.04\) versus \(3.50\)). Reward effects varied across participants, with some participants showing faster responses in high-reward trials and others showing little or opposite reward-related change. (Supplementary Figure~S3).
...
The projected time courses provided an initial indication that GVS affected the vigour network signal in a stimulus-specific manner. Across GVS conditions, the projected vigour network signal showed a different waveform and medication-specific pattern (Supplementary Figure~S12A). For the late-minus-early response feature, GVS7 was the only GVS waveform that differed from sham in the OFF-medication session (Figure~\ref{fig:gvs_behaviour_network_responses} middle row). The active-minus-sham difference was negative, indicating that GVS7 reduced the late relative to the early component of the projected vigour network response. This effect was absent during the ON-medication session, in which no GVS waveform survived correction. The full vigour network feature summaries are shown in Supplementary Figure~S14A,B.

In contrast, the task-activation map projection did not reproduce this behavioural specificity. In the OFF-medication session, significant task-activation map effects appeared for other stimulation conditions rather than for GVS7, including GVS2 and GVS8 in the late-minus-early response feature and GVS6 in the maximum absolute baseline-response feature (Figure~\ref{fig:gvs_behaviour_network_responses} bottom row; Supplementary Figures~S12B and S15A,B). No task-activation map response survived correction in the ON-medication session. Thus, the stimulation condition associated with faster OFF-medication responses was mirrored by the vigour network projection, but not by the task-activation map projection.
...
The analysis code and supporting materials are available in the GitHub repository \href{https://github.com/zahra1379kavian/Motor_Vigour-Variability-Reproducibility}
{\textit{Motor Vigour Variability Reproducibility}}. The datasets generated and/or analyzed during the current study are available from the corresponding author upon reasonable request.
