...
We used a group-level constrained optimisation problem to identify a vigour-related brain network (Figure~\ref{fig:task_analysis_overview}B,C). The analysis searched for a weight vector that projected multivoxel neural activity onto a single scalar signal for each trial. Let \(X_{\beta}\) denote single-trial beta matrix, with columns corresponding to trials, and let \(X_{\mathrm{BOLD}}\) denote the corresponding trial-window BOLD feature matrix. For the beta estimates, the projected neural signal was
\begin{equation}
y_{\beta} = w^\top X_{\beta},
\end{equation}
where \(w\) is the optimised weight vector and \(y_{\beta}\) is the trial-wise neural projection. The same weight vector was also applied to \(X_{\mathrm{BOLD}}\) to constrain local trial-to-trial instability in the BOLD signal. Because direct optimisation over all voxels was computationally impractical, beta and BOLD data were first reduced with expectation-maximisation principal component analysis (EMPCA) \citep{roweis1997algorithms}. An \(80\%\) cumulative explained-variance threshold retained \(14\) components for optimisation, and final weights were projected back to voxel space after model fitting.

The optimisation framework integrated these constraints as weighted penalties, allowing the final projection to balance task engagement, behavioural relevance, spatial coherence, and trial-to-trial stability. These criteria were written as quadratic terms of the form \(w^\top C w\), where each \(C\) matrix summarised one property of the data in the EMPCA-reduced space. Specifically, \(C_{\mathrm{task}}\) penalised weights assigned to weakly task-engaged activity, \(C_{\mathrm{BOLD}}\) penalised large consecutive-trial changes in the projected BOLD features, \(C_{\beta}\) penalised large consecutive-trial changes in the projected beta estimates, and \(C_{\mathrm{smooth}}\) penalised spatially isolated weights by encouraging neighbouring voxels to have similar contributions. Consecutive-trial variability was used rather than total variance because it preserves trial order and penalises abrupt local jumps while allowing slower task-related changes across runs or blocks. Run and block boundaries were excluded from this penalty because they do not represent continuous local temporal dynamics.

The four penalty terms were combined into
\begin{equation}
C_{\mathrm{total}}
...
Behavioural relevance was represented separately. The matrix \(C_b\) encoded alignment between the projected beta signal and RT, whereas \(C_d\) encoded the variance of the projected beta signal and was used for normalisation. The final objective therefore minimised the combined penalties while maximising behavioural alignment with RT:
\begin{equation}
\min_w
...
\mathrm{s.t.}\ \|w\|_2=1 .
\end{equation}
Here, the \(\alpha\) values control the relative weights of the four penalty terms, and \(\gamma\) controls the overall balance between regularisation and behavioural relevance. The negative sign before \(C_b\) reflects that behavioural alignment was maximised, whereas the other terms were penalised. Full definitions of all matrices and derivations are provided in Supplementary Methods.
...
Voxelwise overlap between the optimised vigour network and the standard task-activation map was quantified using the number of shared voxels, the Dice coefficient, and the Jaccard index. ROI-level anatomical coverage was summarised as the percentage of each atlas-defined ROI occupied by the vigour network, the task-activation map, and their same-voxel overlap.
...
Using Pearson's correlation, active GVS produced \(194\) FDR-significant edge changes (Supplementary Figure~S11). This pattern was more balanced, with \(106\) increases and \(88\) decreases. The strongest increases involved left amygdala–left paracentral lobule, right postcentral cortex–left temporal cortex, and left cerebellum–right paracentral lobule. The strongest decreases involved left caudate–right frontal cortex, right amygdala–left precentral cortex, and left thalamus–left temporal cortex. The two metrics did not identify the same set of edges, which is expected because they measure different forms of dependence. However, both analyses showed that GVS perturbed distributed interactions within the final network, repeatedly involving limbic/olfactory, medial-temporal, subcortical, visual, cingulate–temporal, and somatomotor/cerebellar components.

As a control analysis, we repeated the GVS edge-change analysis using the task-activation map instead of the optimised vigour network (Figure~\ref{fig:gvs_edge_changes}B,C). GVS also produced significant edge changes in this task-activation network. However, the detailed pattern was different: the task-activation map showed \(134\) significant mutual-information edges and \(148\) significant Pearson edges, with only \(14\) shared mutual-information edges and \(16\) shared Pearson edges overlapping the significant vigour network edges. This supports a cautious interpretation: GVS affects task-active circuitry in general, but the optimised vigour network shows a partly distinct pattern of GVS-sensitive connectivity.
...
\begin{figure}[!p]
    ...
    \begin{minipage}[t]{\textwidth}
        \noindent\textbf{B}\par\vspace{0.2em}
        \centering
        \includegraphics[width=\linewidth]{main_figures/Figure9.pdf}
    \end{minipage}\hfill
    \caption{Exploratory GVS-related edge changes in the optimised vigour and task-activation networks. \textbf{A}, FDR-significant active--sham mutual-information edge changes are shown for the optimised vigour network. \textbf{B}, The same mutual-information analysis is shown for the task-activation network. \textbf{C}, FDR-significant active--sham Pearson edge changes are shown for the task-activation network. Changes were pooled across subjects, medication states, runs, and active GVS waveforms. Orange edges show increased statistical dependence or linear correlation, and blue edges show decreased dependence or linear correlation. Colour intensity scales with the mean edge-change magnitude. Nodes are labelled by ROI and grouped by anatomical sector.}
